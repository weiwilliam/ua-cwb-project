...
dates_count=0
print(dlist)
# Each Initialiation Time (just 1)
for date in dlist:
    ...
    for eidx,(usrn,ufsn,dmsn) in enumerate(zip(inpath,expufs,expdms)):
        ...
        for fidx,fhr in enumerate(fhrlist):
            ...
            tmpdsmu = tmpds.mean()
            #Cat results using xarray in fhr,exp,init time
            if fidx==0:
               fhrds=tmpds
               fhrmean=tmpdsmu
            else:
               fhrds=xa.concat((fhrds,tmpds),dim='fhr')
               fhrmean=xa.concat((fhrmean,tmpdsmu),dim='fhr')
        if eidx==0:
           expds=fhrds
           expmean=fhrmean
        else:
           expds=xa.concat((expds,fhrds),dim='exp')
           expmean=xa.concat((expmean,fhrmean),dim='exp')
    if dates_count==0:
       allds=expds
       allmean=expmean
    else:
       allds=xa.concat((allds,expds),dim='time')
       allmean=xa.concat((allmean,expmean),dim='time')
    dates_count+=1

allds=allds.assign_coords({'exp':exp_nm,'fhr':fhrlist,'time':dlist})
allmean=allmean.assign_coords({'exp':exp_nm,'fhr':fhrlist,'time':dlist})
#if area!='Glb':
#   tmpds=allds.sel(lon=slice(minlon,maxlon),lat=slice(minlat,maxlat))

for date in dlist:
    outname='%s/%s_Mean_%s.%s.png' %(outputpath,pltvar,area,date)
    fig,ax=plt.subplots()
    set_size(axe_w,axe_h,b=0.13,l=0.1,r=0.95,t=0.95)
    col = ['blue','red']
    exp_leg = ['CTRL','UFS-RRTMG']
    count=0
    tistr='%s Average: %s' %(date,area) 
    for explb in exp_nm:
        plttmp=allmean.sel(time=date,exp=explb)
        print(plttmp)
        cc=col[count]
        cn=ax.plot(plttmp.fhr,plttmp.pltvar.data,linewidth=3.0,c=cc,marker='.')
        count+=1
